...
function WildfeedUI() {
  ...
  this._wirefeed = new Wildfeed("https://thatyear.wilddogio.com/","101252830");
  this._unload = null;
  ...
  self._wirefeed.onStateChange(function(user) {
    self.onLoginStateChange(user);
  });
}

WildfeedUI.prototype._setupHandlers = function() {
  var self = this;
...
WildfeedUI.prototype._go = function(url) {
  window.History.pushState(null, null, url);
};

WildfeedUI.prototype._pageController = function(url) {
...
WildfeedUI.prototype._postHandler = function(e) {
  ...
  self._wirefeed.post(sparkText.val(), function(err, done) {
    if (!err) {
...
WildfeedUI.prototype._handleNewSpark = function(listId, limit, func) {
  var self = this;
  func(
    limit,
...
WildfeedUI.prototype._formatDate = function(date) {
...
WildfeedUI.prototype._editableHandler = function(id, value) {
  if (id == "inputLocation") {
    this._wirefeed.setProfileField("location", value);
  }
  if (id == "inputBio") {
    this._wirefeed.setProfileField("bio", value);
  }
  return true;
};

WildfeedUI.prototype.onLoginStateChange = function(info) {
  this._spinner.stop();
  this._loggedIn = info;
...
WildfeedUI.prototype.logout = function(e) {
  if (e) {
    e.preventDefault();
  }
  this._wirefeed.logout();
  this._loggedIn = false;
  this.renderHome();
};

WildfeedUI.prototype.render404 = function() {
  // TODO: Add 404 page.
  this.renderHome();
};

WildfeedUI.prototype.goHome = function() {
  this._go("/");
};

WildfeedUI.prototype.renderHome = function(e) {
  if (e) {
    e.preventDefault();
  }
  if (this._loggedIn) {
  ...
  self._handleNewSpark(
    "spark-index-list", 5,
    self._wirefeed.onLatestSpark.bind(self._wirefeed)
  );
  return function() { self._wirefeed.unload(); };
};

WildfeedUI.prototype.renderSearch = function() {
  var self = this;
  ...
  self._wirefeed.startSearch(function(results) {
  ...
  var onCharChange = function() {
    ...
    self._wirefeed.updateSearchTerm(searchTerm);
  };
...
WildfeedUI.prototype.renderTimeline = function(info) {
  var self = this;
  $("#header").html(Mustache.to_html($("#tmpl-page-header").html(), {user: self._loggedIn}));
  ...
  self._handleNewSpark(
    ...
    self._wirefeed.onNewSpark.bind(self._wirefeed)
  );

  // Get some "suggested" users.
  self._wirefeed.getSuggestedUsers(function(userid, info) {
    info.id = userid;
    ...
    button.click(function(e) {
      ...
      e.preventDefault();
      $button.remove();
      self._wirefeed.follow(id, function(err, done) {
...
WildfeedUI.prototype.renderProfile = function(uid) {
  var self = this;
  var facebookId = uid.replace('qq:', '');
  $("#header").html(Mustache.to_html($("#tmpl-page-header").html(), {user: self._loggedIn}));
  ...
  // Update user info.
  self._wirefeed.getUserInfo(uid, function(info) {
    info.id = uid;
    ...
    if (self._loggedIn && self._loggedIn.id != info.id) {
      button.click(function(e) {
        ...
        self._wirefeed.follow(clickedButtonId, function(err, done) {
          // TODO FIXME: Check for errors!
  ...
  self._handleNewSpark(
    ...
    self._wirefeed.onNewSparkFor.bind(self._wirefeed, uid)
  );
  return function() { self._wirefeed.unload(); };
};

WildfeedUI.prototype.renderSpark = function(id) {
  var self = this;
  $("#header").html(Mustache.to_html($("#tmpl-page-header").html(), {user: self._loggedIn}));

  // Render spark page body.
  self._wirefeed.getSpark(id, function(spark) {
    if (spark !== null && spark.author) {
      self._wirefeed.getUserInfo(spark.author, function(authorInfo) {
