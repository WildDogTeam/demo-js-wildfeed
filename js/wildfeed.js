...
function Wildfeed(baseURL,qqAppId, newContext) {
  var self = this;
  this._name = null;
  this._qqId = null;
  this._wilddog = null;
  ...
  this._wilddog = new Wilddog(
    baseURL, newContext || false ? new Wilddog.Context() : null
  );

  this._authHandlers = [];
  this._wilddog.onAuth(self._onLoginStateChange.bind(self));
}
Wildfeed.prototype = {
  ...
  _getPicURL: function(id, large) {
    ...
    //also can get info by user.qq.CacheUserProfile.figureurl
  ...
  onStateChange: function(cb) {
    this._wilddog.onAuth(cb.bind(this));
  }
};
 ...
 */
Wildfeed.prototype.onLoginStateChange = function(onLoginStateChange) {
  var self = this;
...
/**
 * Login a user using Wilddog Simple Login, using the specified authentication
 * provider. Pass the optional 'rememberMe' argument to the WilddogSimpleLogin
 ...
 */
Wildfeed.prototype.logout = function() {
  if (this._uid) {
    // Set presence to offline, reset all instance variables, and return!
    var peopleRef = this._wilddog.child("people").child(this._uid);
    peopleRef.child("presence").set("offline");
  }
  this._wilddog.unauth();
};

/**
 * On successful authentication, set up Wilddog references and hang on to
 * relevant user data like id and name. Wilddog Simple Login automatically
...
Wildfeed.prototype.onLogin = function(user) {
  ...
  this._wilddog.child('search/firstName').child(firstNameKey).set(user['id']);
  this._wilddog.child('search/lastName').child(lastNameKey).set(user['id']);

  this._mainUser = self._wilddog.child("users").child(this._uid);
  this._fullName = user.name;
  this._name = user.first_name;
 
  var peopleRef = self._wilddog.child("people").child(this._uid);
  peopleRef.once("value", function(peopleSnap) {
    ...
    if (!val) {
      ...
      info = {
        ...
        location: "",
        bio: "",
        pic: self._getPicURL(self._qqId)
      };
      peopleRef.set(info);
    } else {
      info = val;
 ...
 * the current user as offline. Wilddog Simple Login automatically handles
 * user sessions, so there is no need to do any additional sessioning here.
 */
Wildfeed.prototype.onLogout = function() {
  this._user = null;
  this._qqId = null;
  this._mainUser = null;
  this._fullName = null;
 ...
 */
Wildfeed.prototype.getUserInfo = function(user, onComplete,
                                          onFollower, onFollowersComplete,
                                          onFollowee, onFolloweesComplete) {
  var self = this;
  self._validateCallback(onComplete, true);

  var ref = self._wilddog.child("people").child(user);
  var handler = ref.on("value", function(snap) {
    var val = snap.val();
    val.pic = self._getPicURL(snap.key(), true);
  ...
  var userRef = self._wilddog.child('users').child(user);
  var followerRef = userRef.child('followers');
  var followerHandle = followerRef.on('child_added', function(snapshot) {
    self._wilddog.child('people').child(snapshot.key()).once('value', function(snap) {
...
Wildfeed.prototype.startSearch = function(resultsHandler) {
  this._searchHandler = resultsHandler;
};

Wildfeed.prototype.updateSearchTerm = function(term) {
  ...
  if (isValidStem(term) && term.length >= 3) {
    if (this._currentSearch) {
      ...
      this._currentSearch = new WildfeedSearch(this._wilddog, term, this._searchHandler);
    }
  } else {
    this.stopSearching();
  }
...
Wildfeed.prototype.stopSearching = function() {
  if (this._currentSearch) {
 ...
 */
Wildfeed.prototype.getSpark = function(id, onComplete) {
  var self = this;
  self._validateCallback(onComplete, true);
  self._wilddog.child("sparks").child(id).once("value", function(snap) {
    onComplete(snap.val());
  });
};
 ...
 */
Wildfeed.prototype.follow = function(user, onComplete) {
  var self = this;
  ...
  self._mainUser.child("following").child(user).set(true, function(err) {
    ...
    var followUser = self._wilddog.child("users").child(user);
    ...
    followUser.child("sparks").once("value", function(sparkSnap) {
      sparkSnap.forEach(function(spark) {
        myFeed.child(spark.key()).set(true);
      });
    });
 ...
 */
Wildfeed.prototype.post = function(content, onComplete) {
  var self = this;
  ...
  var sparkRef = self._wilddog.child("sparks").push();
  var sparkRefId = sparkRef.key();
  var spark = {
  ...
  sparkRef.set(spark, function(err) {
    ...
    feedSparkRef.set(true, function(err) {
      ...
      var recentUsersRef = self._wilddog.child("recent-users");
      ...
      var recentSparkRef = self._wilddog.child("recent-sparks");
      ...
      self._mainUser.child("followers").once("value", function(followerList) {
        followerList.forEach(function(follower) {
          ...
          var childRef = self._wilddog.child("users").child(follower.key());
 ...
 *                                        suggested user.
 */
Wildfeed.prototype.getSuggestedUsers = function(onSuggestedUser) {
  var self = this;
  ...
  self._mainUser.child("following").once("value", function(followSnap) {
    ...
    // already following.
    var recentUsersQuery = self._wilddog.child("recent-users").limitToFirst(20);
    var count = 0;
 ...
 */
Wildfeed.prototype.setProfileField = function(field, value) {
  var peopleRef = this._wilddog.child("people").child(this._uid);
 ...
 * spark (see Wildfeed.post), which will appear in real-time on the current
 * user's feed!
 *
 ...
 */
Wildfeed.prototype.onNewSpark = function(totalCount, onComplete, onOverflow) {
  this._validateCallback(onComplete);
  this._validateCallback(onOverflow);

  var feed = this._mainUser.child("feed").limitToFirst(totalCount || 100);
 ...
 */
Wildfeed.prototype.onNewSparkFor = function(id, count, onComplete, onOverflow) {
  this._validateCallback(onComplete, true);
  this._validateCallback(onOverflow, true);

  var feed = this._wilddog.child("users").child(id).child("sparks");
  feed = feed.limitToFirst(count || 10);
 ...
 *                                   latest set.
 */
Wildfeed.prototype.onLatestSpark = function(count, onComplete, onOverflow) {
  this._validateCallback(onComplete, true);
  this._validateCallback(onOverflow, true);

  var feed = this._wilddog.child("recent-sparks");
  feed = feed.limitToFirst(count || 5);
 ...
 * reuse the Wildfeed object after calling this and registering new handlers.
 */
Wildfeed.prototype.unload = function() {
